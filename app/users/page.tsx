...
import { USERS_MOCK_DATA } from "@/mocks/users.mock";

export default function UsersPage() {
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-bold tracking-tight">Users & Staff</h2>
        <p className="text-muted-foreground text-sm">
          Manage all users, their roles, and access across clinics.
        </p>
      </div>
      ...
      <UserTable users={USERS_MOCK_DATA} />

      <div className="flex items-center justify-between px-2">
        <div className="flex-1 text-sm text-muted-foreground">
          Showing <strong>1</strong> to <strong>5</strong> of{" "}
          <strong>{USERS_MOCK_DATA.length}</strong> users
        </div>
